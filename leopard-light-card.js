class LeopardLightCard extends HTMLElement {
  ...
  _handleDragMove(e) {
    ...
    if (status)
      status.textContent = `${Math.round(this._currentBrightness)}%`;
  ...
  _handleDragEnd() {
    ...
    if (this._hasMoved) {
      this._setBrightness(this._currentBrightness);
    }
...
class LeopardLightCardEditor extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot
      .querySelectorAll('ha-entity-picker, ha-icon-picker, ha-slider')
      .forEach((el) => {
        el.addEventListener('click', (e) => e.stopPropagation());
        el.addEventListener('mousedown', (e) => e.stopPropagation());

        el.addEventListener('value-changed', (e) => {
          const key = e.target.configValue;
